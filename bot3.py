...
async def boshliq_orqaga_handler(update, context):
    """Boshliq panelidagi '⬅️ Orqaga' tugmasi boshliq menyusiga qaytaradi"""
    if update.callback_query:
        await update.callback_query.answer()
        await boshliq_menu(update.callback_query, context)
    else:
        await boshliq_menu(update.message, context)
...
# ================== BOSHLIQ: XABARLAR (BOSHLQ TOMONI) ==================
async def boss_xabar_menu(update, context):
    """
    Boshliq '📝 Xabarlar' tugmasini bosganda — ishchilardan kelgan xabarlarni ko‘rsatadi.
    """
    conn = sqlite3.connect("ishchilar.db")
    cur = conn.cursor()
    cur.execute("""
        SELECT fullname, passport, content, date, is_read 
        FROM messages 
        JOIN ishchilar ON messages.passport = ishchilar.passport
        WHERE sender='worker'
    ...
    if not rows:
        text = "📭 Hali ishchilardan xabar kelmagan."
    else:
        text = "📨 Oxirgi ishchi xabarlari:\n\n"
        for fullname, passport, content, date, is_read in rows:
            prefix = "✅" if is_read else "✉️"
            text += f"{prefix} <b>{fullname}</b> ({passport})\n🕒 {date}\n{content}\n\n"

    kb = ReplyKeyboardMarkup([["✉️ Yangi xabar yuborish"], ["⬅️ Orqaga"]], resize_keyboard=True)
    await update.message.reply_text(text, reply_markup=kb, parse_mode="HTML")


async def boss_send_message(update, context):
    """
    Boshliq tomonidan ishchiga xabar yuborish.
    """
    text = update.message.text
    if text == "✉️ Yangi xabar yuborish":
        # ishchi tanlash ro‘yxatini chiqaramiz
        conn = sqlite3.connect("ishchilar.db")
        cur = conn.cursor()
        cur.execute("SELECT id, fullname FROM ishchilar WHERE role='worker'")
        workers = cur.fetchall()
        conn.close()
        ...
        buttons = [
            [InlineKeyboardButton(name, callback_data=f"msg_{wid}")]
            for wid, name in workers
        ]
        buttons.append([InlineKeyboardButton("⬅️ Bekor qilish", callback_data="back_boss")])

        await update.message.reply_text(
            "Kimga xabar yubormoqchisiz?",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
        context.user_data["awaiting_boss_message"] = False
        return

    # Agar oldingi bosqichda ishchi tanlangan bo‘lsa
    if context.user_data.get("awaiting_boss_message"):
        ishchi_id = context.user_data.get("selected_worker_id")
        conn = sqlite3.connect("ishchilar.db")
        cur = conn.cursor()
        cur.execute("SELECT passport, fullname, tg_id FROM ishchilar WHERE id=?", (ishchi_id,))
        row = cur.fetchone()
        conn.close()
        ...
        passport, fullname, tg_id = row

        # bazaga saqlaymiz
        conn = sqlite3.connect("ishchilar.db")
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO messages (ishchi_id, passport, sender, type, content, date, is_read) VALUES (?, ?, 'boss', 'text', ?, ?, 0)",
            (ishchi_id, passport, text, datetime.now().strftime("%Y-%m-%d %H:%M"))
        )
        conn.commit()
        conn.close()
        ...
        context.user_data["awaiting_boss_message"] = False
        await update.message.reply_text(f"✅ Xabar {fullname} ga yuborildi.")
        return

    # Aks holda hech narsa tanlanmagan
    await update.message.reply_text("Iltimos, kimga yuborishni tanlang (✉️ Yangi xabar yuborish).")


# === Callback orqali ishchini tanlash ===
async def boss_select_message_target(update, context):
    ...
    if data.startswith("msg_"):
        ishchi_id = int(data.split("_")[1])
        context.user_data["selected_worker_id"] = ishchi_id
        context.user_data["awaiting_boss_message"] = True
        await q.message.reply_text("✍️ Xabar matnini kiriting:")
...
# === Ishchi o‘chirish ===
from telegram import InlineKeyboardButton, InlineKeyboardMarkup
import asyncio  # Hozircha ishlatilmayapti, lekin kerak bo‘lishi mumkin

# === Ishchilar ro‘yxati va o‘chirish menyusi ===
async def ishchi_ochirish(update, context):
    ...
    if not workers:
        if update.callback_query:
            await update.callback_query.message.edit_text("Ishchilar mavjud emas ❌")
        else:
...
async def delete_worker(update, context):
    ...
    q = update.callback_query
    await q.answer()  # Bu tugmani bosilganda "loading" holatini olib tashlaydi

    # 🔙 Orqaga bosilsa boshliq menyusiga qaytish
    if q.data == "back_boss":
        # Eski xabarni edit qilamiz va boshliq menyusini ko‘rsatamiz
        await q.message.edit_text(
            "👔 Boshliq menyusi:",
            reply_markup=boshliq_buttons()  # Sizning boshliq menyu tugmalaringiz funksiyasi
        )
        return

    # Callback_data dan ishchi ID ni ajratib olish
    try:
...
# === Boshliq menyusi tugmalari (misol) ===
def boshliq_buttons():
    """Boshliq menyusi tugmalari"""
    buttons = [
        [InlineKeyboardButton("📨 Ish berish", callback_data="ish_berish")],
        [InlineKeyboardButton("🗂 Ishchilar ro‘yxati", callback_data="ishchilar")],
        [InlineKeyboardButton("🗑 Ishchi o‘chirish", callback_data="ochirish")],
    ]
...
def main():
    ...
    # ====== Ishchi xabar menu ======
    app.add_handler(
        MessageHandler(filters.Regex("^📝 Xabar$") & ~filters.COMMAND, ishchi_xabar_menu)
    )
    ...
    # ====== Boss xabarlar paneli ======
    app.add_handler(MessageHandler(filters.Regex("^📝 Xabarlar$") & ~filters.COMMAND, boss_xabar_menu))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, boss_send_message))
    app.add_handler(CallbackQueryHandler(boss_select_message_target, pattern="^(msg_|back_boss)$"))
